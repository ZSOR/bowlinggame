// <auto-generated />
using System;
using BowlingGame.DAL;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace BowlingGame.Migrations
{
    [DbContext(typeof(BowlingDbContext))]
    [Migration("20220105145605_Adding foreign key to players")]
    partial class Addingforeignkeytoplayers
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.1")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("BowlingGame.DAL.Models.FrameDTO", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int?>("ScoreCardDTOId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ScoreCardDTOId");

                    b.ToTable("FrameDTO");
                });

            modelBuilder.Entity("BowlingGame.DAL.Models.GameDTO", b =>
                {
                    b.Property<int>("ID")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.HasKey("ID");

                    b.ToTable("Games");
                });

            modelBuilder.Entity("BowlingGame.DAL.Models.PlayerDTO", b =>
                {
                    b.Property<int>("Guid")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.Property<int>("GameId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("longtext");

                    b.Property<int>("ScoreCardId")
                        .HasColumnType("int");

                    b.HasKey("Guid");

                    b.HasIndex("GameId");

                    b.HasIndex("ScoreCardId");

                    b.ToTable("Players");
                });

            modelBuilder.Entity("BowlingGame.DAL.Models.ScoreCardDTO", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("ScoreCardDTO");
                });

            modelBuilder.Entity("BowlingGame.DAL.Models.FrameDTO", b =>
                {
                    b.HasOne("BowlingGame.DAL.Models.ScoreCardDTO", null)
                        .WithMany("Frames")
                        .HasForeignKey("ScoreCardDTOId");
                });

            modelBuilder.Entity("BowlingGame.DAL.Models.PlayerDTO", b =>
                {
                    b.HasOne("BowlingGame.DAL.Models.GameDTO", "Game")
                        .WithMany("Players")
                        .HasForeignKey("GameId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("BowlingGame.DAL.Models.ScoreCardDTO", "ScoreCard")
                        .WithMany()
                        .HasForeignKey("ScoreCardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Game");

                    b.Navigation("ScoreCard");
                });

            modelBuilder.Entity("BowlingGame.DAL.Models.GameDTO", b =>
                {
                    b.Navigation("Players");
                });

            modelBuilder.Entity("BowlingGame.DAL.Models.ScoreCardDTO", b =>
                {
                    b.Navigation("Frames");
                });
#pragma warning restore 612, 618
        }
    }
}
